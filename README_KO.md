...
두 공개 데이터셋 모두 우리가 평가에 사용한 것보다 큽니다. 아래 표는 "전체 데이터셋 중 얼마나 사용했는지, README의 정확도가 실제로 얼마만큼의 테스트 데이터에서 나왔는지"에 대한 직접적인 답입니다:

| 대상 행동 | 전체 공개 데이터셋 | 실제 사용량 (train+test) | 발표된 정확도의 근거가 된 테스트 데이터 |
|---|---|---|---|
| 낙상 | UP-Fall — 총 **17명** 피험자 | **17명 중 4명** (Subject 1–4), 전체 11개 activity, 전체 trial | **69,150 프레임** — 이 4명의 모든 프레임이 4개 LOOCV fold에 걸쳐 정확히 한 번씩 테스트됨 |
| 무활동 | UP-Fall — 총 **17명** 피험자 (낙상과 동일한 출처) | 동일한 **17명 중 4명**, activity 6–9만 (걷기/서있기/앉기/물건줍기) | **40,500 프레임** — 동일한 4-fold LOOCV, 모든 프레임이 한 번씩 테스트됨 |
| 달리기 | KTH — 총 **25명** 피험자 | **25명 중 25명** (100% — 일부만 뽑지 않음) | **200개 비디오 클립** — 모든 클립이 2개 LOOCV fold에 걸쳐 정확히 한 번씩 테스트됨 |

> 낙상과 무활동은 UP-Fall의 17명 중 의도적으로 4명만 사용합니다 (Subject 5–17은 train이든 test든 전혀 사용되지 않음). 달리기는 KTH 데이터셋 전체를 사용합니다. 위에서 "사용됨"이라고 표시된 피험자/클립이 곧 README의 92.40% / 95.83% / 90.99%가 산출된 테스트 데이터이기도 합니다 — LOOCV에서는 사용된 모든 피험자나 클립이 정확히 한 번씩 held-out되어 채점되기 때문입니다.
...
> **git에 커밋되지 않음.** 이 폴더들은 총 ~10GB(원본 PNG 프레임이 용량이 큼)로, git 저장소가 감당할 범위를 훨씬 넘습니다. 그래서 `data/`는 계속 `.gitignore`에 남습니다. git이 아닌 **클라우드 드라이브**를 통해 공유됩니다: **[테스트 데이터 폴더 (Google Drive)](https://drive.google.com/drive/folders/1ZT5d8DihBuWDdCotHD6W1OxRpeF71I_V?usp=drive_link)** — `git clone` 후에는 자동으로 포함되지 않습니다.
