"""
Compute biomechanical features from a sequence of PoseFrame objects.

Pose format: YOLO11n-pose, 17 COCO joints, kp[i] = (x_norm, y_norm, conf).
All spatial values are in normalized image coordinates [0, 1].
...
def body_tilt_angle(kps: np.ndarray) -> float:
    """
    Angle of spine (mid_hip → mid_shoulder) from vertical (degrees).
    ...
    vertical = np.array([0.0, -1.0])   # upward in image coords (y=0 is top)
    return _angle_deg(spine, vertical)


def hip_height_normalized(kps: np.ndarray) -> float:
    """
    Y-coordinate of mid-hip (0=top, 1=bottom of frame).
    Large value → hips are low → possible fall.
    """
...
def pose_aspect_ratio(kps: np.ndarray) -> float:
    """
    Bounding-box height / width of all visible keypoints.
    Low value (<1) → person is wide/horizontal.
    """
...
class FeatureBuffer:
    """
    Sliding window of per-frame feature dicts.
    Exposes aggregate temporal features: speed, step frequency, variance.
    """

    def __init__(self, window_frames: int = 30, fps: float = 19.0):
    ...
    def step_frequency(self) -> float:
        """
        Step frequency (Hz) via zero-crossings of (L_knee_angle - R_knee_angle).
        Each crossing = half a step cycle.
        """
        v = self._valid()
        if len(v) < 6:
            return 0.0
    ...
    def mean_keypoint_displacement(self) -> float:
        """
        Mean per-frame displacement of all visible keypoints.
        Near-zero → person is stationary.
        """
        v = self._valid()
        if len(v) < 2:
            return 0.0
        ...
        for i in range(1, len(v)):
            kp0 = v[i-1]["kps"]
            kp1 = v[i]["kps"]
            mask = (kp0[:, 2] >= KP_VISIBILITY_THRESHOLD) & \
                   (kp1[:, 2] >= KP_VISIBILITY_THRESHOLD)
            if mask.sum() == 0:
                continue
